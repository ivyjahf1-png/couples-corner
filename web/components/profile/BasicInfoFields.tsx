...
export function BasicInfoFields({ formData, updateField, errors }: FieldGroupProps) {
  return (
    <Card as="section" className="flex flex-col gap-4" aria-label="About">
      ...
      <div>
        <label htmlFor="displayName" className="mb-1.5 block text-sm font-medium text-ink-200">
          Display name
        </label>
        <input
          id="displayName"
        ...
        />
        {errors?.displayName ? (
          <p id="displayName-error" className="mt-1 text-xs text-danger-300">{errors.displayName}</p>
        ) : null}
      </div>
      <div>
        <label htmlFor="bio" className="mb-1.5 block text-sm font-medium text-ink-200">
          Bio
        </label>
        <textarea
          id="bio"
          ...
          maxLength={500}
          className="w-full resize-none rounded-xl border border-ink-700 bg-surface px-4 py-2.5 text-sm text-white focus:border-brand-500/60 focus:outline-none"
          placeholder="Tell people what makes you, you."
          aria-invalid={!!errors?.bio}
          aria-describedby={errors?.bio ? "bio-error" : undefined}
        />
        {errors?.bio ? (
          <p id="bio-error" className="mt-1 text-xs text-danger-300">{errors.bio}</p>
        ) : null}
      </div>
    </Card>
  );
