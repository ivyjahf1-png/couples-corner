...
export function CallOverlay({ mode, summary, conversationId, currentUserId, onClose }: { mode: CallMode; summary: ConversationParticipantSummary | null; conversationId: string; currentUserId: string; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] flex flex-col bg-slate-950/95 backdrop-blur-xl" role="dialog" aria-modal="true" aria-label={`${mode} call`}>
    {/* Safe-area top padding: this overlay is `fixed inset-0`, so it covers the
        FULL viewport including the notch/status-bar strip that the chat header
        insets itself around. Without the floor it would render its title row
        underneath the status bar. `max()` keeps the existing 0.75rem breathing
        room on devices with no inset and grows to the full inset where one
        exists — the same treatment as ChatHeader, so the two agree. */}
    <div className="flex shrink-0 items-center justify-between border-b border-slate-800/80 bg-slate-950/90 px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-3 backdrop-blur-md sm:px-6"><div><p className="text-xs uppercase tracking-widest text-purple-300">{mode} call · {call.status}</p><h2 className="font-semibold text-white">{summary?.name ?? "Chat"}</h2></div><span className="rounded-full bg-white/10 px-3 py-1 text-sm tabular-nums text-white/80">{time}</span></div>
