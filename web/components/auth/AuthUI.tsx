  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-background px-4 py-12">
      ...
      <div className="w-full max-w-md rounded-2xl border border-ink-200 bg-surface p-8 shadow-card">
        ...
        <p className="mt-2 text-sm leading-6 text-ink-300">{subtitle}</p>
        <div className="mt-6">{children}</div>
      </div>
      {footer ? <div className="mt-6 text-sm text-ink-300">{footer}</div> : null}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        {isPassword ? (
          <button
            ...
            className="absolute inset-y-0 right-0 flex items-center justify-center rounded-r-xl px-3 text-ink-300 transition-colors hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"
          >
      ...
      {error ? (
        ...
        <p id={`${id}-hint`} className="text-xs text-ink-300">
          {hint}
        </p>
      ) : null}
    </div>
